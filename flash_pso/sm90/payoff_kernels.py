"""Payoff evaluation kernels (SM90 — TMA loads for ln_positions and St).

SM90 differences from SM80:
  - TMA load for ln_positions when BLOCK_SIZE_PARTICLES >= 4 (Both SCALAR and PER_ASSET).
  - TMA load for st in bandwidth branch.
  - Flattened TMA descriptor for per-asset st (EXERCISE_STYLE=1).
  - Same helper-function structure, tl.dot Cholesky, tl.min for per-asset AND.
"""
import triton
...
def process_dim_block_compute_vanilla(
    ...
    ln_positions_ptr, ln_pos_desc, dim_offset, p_idx, pid, NUM_PARTICLES: tl.constexpr,
    ...
    for d in tl.static_range(BLOCK_SIZE_DIM):
        ...
        if BLOCK_SIZE_PARTICLES == 1:
            ...
            if BLOCK_SIZE_PARTICLES >= 4:
                pos_2d = tl.broadcast_to(tl.load_tensor_descriptor(ln_pos_desc, [step_idx, pid * BLOCK_SIZE_PARTICLES]), [BLOCK_SIZE_PATHS, BLOCK_SIZE_PARTICLES])
            else:
                pos_2d = tl.broadcast_to(tl.expand_dims(tl.load(ln_positions_ptr + step_idx * NUM_PARTICLES + p_idx), 0), [BLOCK_SIZE_PATHS, BLOCK_SIZE_PARTICLES])
            any_ex = (pos_2d > current_lnS[:, None]) if OPTION_TYPE == 1 else (pos_2d < current_lnS[:, None])
            just_ex = any_ex & ~done_acc
...
def process_dim_block_bandwidth_vanilla(
    current_lnS, done_acc, ex_lnS_acc, ex_step_acc,
    ln_positions_ptr, ln_pos_desc, st_desc, dim_offset, p_idx, pid, bw_block_idx,
    ...
    for d in tl.static_range(BLOCK_SIZE_DIM):
        ...
        current_lnS = tl.reshape(st_tile, [BLOCK_SIZE_PATHS])
...
# ══════════════════════════════════════════════════════════════════════════════
# ── Vanilla American payoff (SM90 TMA) ───────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
        ...
        if NUM_BW_PATHS == NUM_PATHS:
            ...
            if is_compute:
                current_lnS, done_acc, ex_lnS_acc, ex_step_acc = process_dim_block_compute_vanilla(
                    current_lnS, done_acc, ex_lnS_acc, ex_step_acc,
                    ln_positions_ptr, ln_pos_desc, dim_offset, p_idx, pid, NUM_PARTICLES,
                    seed, base_rng_offsets, drift_l2, vol_l2,
    ...
    for d in tl.static_range(BLOCK_SIZE_DIM):
        ...
        if BLOCK_SIZE_PARTICLES == 1:
            ...
            if BLOCK_SIZE_PARTICLES >= 4:
                ...
                pos_2d = tl.broadcast_to(tl.expand_dims(tl.load(ln_positions_ptr + step_idx * NUM_PARTICLES + p_idx), 0), [BLOCK_SIZE_PATHS, BLOCK_SIZE_PARTICLES])
            any_ex = (pos_2d > step_ln_avg[:, None]) if OPTION_TYPE == 1 else (pos_2d < step_ln_avg[:, None])
            just_ex = any_ex & ~done_acc
...
# ══════════════════════════════════════════════════════════════════════════════
# ── American Asian payoff (SM90 TMA) ─────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for d in tl.static_range(BLOCK_SIZE_DIM):
        ...
        if EXERCISE_STYLE == 0:
            if BLOCK_SIZE_PARTICLES == 1:
                ...
                if BLOCK_SIZE_PARTICLES >= 4:
                    ln_pos_slice = tl.reshape(tl.load_tensor_descriptor(ln_pos_desc, [step_idx, pid * BLOCK_SIZE_PARTICLES]), [1, BLOCK_SIZE_PARTICLES])
                else:
                    ln_pos_slice = tl.load(ln_positions_ptr + step_idx * NUM_PARTICLES + p_idx + tl.arange(0, BLOCK_SIZE_PARTICLES))[None, :]
                any_ex = (basket_lnS[:, None] < ln_pos_slice) if OPTION_TYPE == 1 else (basket_lnS[:, None] > ln_pos_slice)
                just_ex = any_ex & ~done_acc
            ...
            if BLOCK_SIZE_PARTICLES == 1:
                ...
                if BLOCK_SIZE_PARTICLES >= 4:
                    ln_bounds_a = tl.reshape(tl.load_tensor_descriptor(ln_pos_desc, [step_idx * NUM_ASSETS, pid * BLOCK_SIZE_PARTICLES]), [NUM_ASSETS, BLOCK_SIZE_PARTICLES])
                else:
                    bound_offs = (step_idx * NUM_ASSETS + a_offs_1d) * NUM_PARTICLES
                    p_offs = tl.arange(0, BLOCK_SIZE_PARTICLES)
                    ln_bounds_a = tl.load(ln_positions_ptr + bound_offs[:, None] + (p_idx + p_offs)[None, :])
...
def process_dim_block_bandwidth_basket(
    current_lnS, done_acc, ex_S_acc, ex_step_acc,
    ln_positions_ptr, ln_pos_desc, st_ptr, st_perasset_desc, dim_offset, p_idx, pid, NUM_PARTICLES: tl.constexpr,
    NUM_BW_PATHS: tl.constexpr, bw_path_offs, bw_block_idx, weights_exp, a_offs_1d,
    BLOCK_SIZE_DIM: tl.constexpr, BLOCK_SIZE_PATHS: tl.constexpr, BLOCK_SIZE_PARTICLES: tl.constexpr, NUM_ASSETS: tl.constexpr,
    ...
    for d in tl.static_range(BLOCK_SIZE_DIM):
        ...
        if EXERCISE_STYLE == 0:
            ...
            st_tile = tl.load_tensor_descriptor(st_perasset_desc, [step_idx * NUM_ASSETS, bw_block_idx * BLOCK_SIZE_PATHS])
            current_lnS = tl.reshape(st_tile, [NUM_ASSETS, BLOCK_SIZE_PATHS])
            S_matrix = tl.exp2(current_lnS)
            basket_S = tl.sum(S_matrix * weights_exp, axis=0)
...
def mc_basket_payoff_kernel(
    ...
    if EXERCISE_STYLE == 1 and NUM_BW_PATHS > 0:
    ...
    for dim_block_idx in tl.range(NUM_DIMENSIONS // BLOCK_SIZE_DIM, num_stages=LOOP_STAGES, loop_unroll_factor=LOOP_UNROLL, warp_specialize=WARP_SPECIALIZE, flatten=LOOP_FLATTEN):
        ...
        if NUM_BW_PATHS == NUM_PATHS:
            current_lnS, done_acc, ex_S_acc, ex_step_acc = process_dim_block_bandwidth_basket(
                current_lnS, done_acc, ex_S_acc, ex_step_acc,
                ln_positions_ptr, ln_pos_desc, st_ptr, st_perasset_desc, dim_offset, p_idx, pid, NUM_PARTICLES,
                NUM_BW_PATHS, bw_path_offs, bw_block_idx, weights_exp, a_offs_1d,
