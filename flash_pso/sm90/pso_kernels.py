...
def init_kernel(
    ...
    if GENERATES_POSITIONS:
        ...
        if OPTION_TYPE == 1:
            # PUT: Spawn flat boundaries between 70% and 100% of the strike
            pos = center * (0.7 + 0.3 * particle_rand)
        else:
            # CALL: Spawn flat boundaries between 100% and 130% of the strike
            pos = center * (1.0 + 0.3 * particle_rand)
...
# ══════════════════════════════════════════════════════════════════════════════
# SM90 MC PAYOFF — TMA descriptors for ln_positions and st
# ══════════════════════════════════════════════════════════════════════════════
            ...
            if is_compute:
                for d in tl.static_range(BLOCK_SIZE_DIM):
                    ...
                    if BLOCK_SIZE_PARTICLES == 1:
                        ...
                        ex_lnS_acc = tl.where(just_ex, step_lnS, ex_lnS_acc); ex_step_acc = tl.where(just_ex, step_idx, ex_step_acc); done_acc = done_acc | any_ex
                    else:
                        if BLOCK_SIZE_PARTICLES >= 4:
                        ...
                        just_ex = any_ex & ~done_acc
                        ex_lnS_acc = tl.where(just_ex, lnS_2d, ex_lnS_acc); ex_step_acc = tl.where(just_ex, step_idx, ex_step_acc); done_acc = done_acc | any_ex
            else:
                for d in tl.static_range(BLOCK_SIZE_DIM):
                    step_idx = dim_offset + d
                    # ── SM90: TMA load for st ────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════════
# SM90 MC ASIAN PAYOFF — TMA descriptors
# ══════════════════════════════════════════════════════════════════════════════
@triton.autotune(configs=get_autotune_configs(), key=["NUM_PARTICLES", "NUM_PATHS", "NUM_DIMENSIONS", "NUM_COMPUTE_PATH_BLOCKS"], warmup=2, rep=3)
@triton.jit
            ...
            if is_compute:
                for d in tl.static_range(BLOCK_SIZE_DIM):
                    ...
                    if BLOCK_SIZE_PARTICLES == 1:
                        ...
                        just_ex = any_ex & ~done_acc
                        ex_avg_acc = tl.where(just_ex, step_avg, ex_avg_acc); ex_step_acc = tl.where(just_ex, step_idx, ex_step_acc); done_acc = done_acc | any_ex
                    else:
                        if BLOCK_SIZE_PARTICLES >= 4:
                            ln_pos_tile = tl.load_tensor_descriptor(ln_pos_desc, [step_idx, pid * BLOCK_SIZE_PARTICLES])
                            pos_2d = tl.broadcast_to(ln_pos_tile, [BLOCK_SIZE_PATHS, BLOCK_SIZE_PARTICLES])
                        ...
                        just_ex = any_ex & ~done_acc
                        ex_avg_acc = tl.where(just_ex, avg_2d, ex_avg_acc); ex_step_acc = tl.where(just_ex, step_idx, ex_step_acc); done_acc = done_acc | any_ex
            else:
                for d in tl.static_range(BLOCK_SIZE_DIM):
                    step_idx = dim_offset + d
                    st_tile = tl.load_tensor_descriptor(st_desc, [step_idx, bw_block_idx * BLOCK_SIZE_PATHS])
                    step_lnS = tl.reshape(st_tile, [BLOCK_SIZE_PATHS]); current_lnS = step_lnS
                    step_S = tl.exp2(current_lnS); running_sum = running_sum + step_S
                    step_avg = running_sum / (step_idx + 1.0); step_ln_avg = tl.log2(step_avg)
                    if BLOCK_SIZE_PARTICLES == 1:
                        ln_pos_slice = tl.load(ln_positions_ptr + step_idx * NUM_PARTICLES + p_idx)
