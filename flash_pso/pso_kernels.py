...
def pso_kernel(
    ...
    st_ptr,                 # [NUM_DIMENSIONS, NUM_BW_PATHS] col-major, bandwidth portion only
    r1_ptr,
    r2_ptr,
    ...
    pid            = tl.program_id(0)
    path_block_idx = tl.program_id(1)
    ...
    path_desc = tl.make_tensor_descriptor(
        base=st_ptr,
        shape=[NUM_DIMENSIONS, NUM_BW_PATHS],        # pyright: ignore[reportArgumentType]
        strides=[NUM_BW_PATHS, 1],                   # pyright: ignore[reportArgumentType]
    ...
    for dim_block_idx in range(NUM_DIMENSIONS // BLOCK_SIZE_DIM):
        ...
        if is_compute:
            if BLOCK_SIZE_DIM == 1:
                rng_offs_mc = path_offs * NUM_DIMENSIONS + dim_offset
                Z_1d        = tl.randn(SEED, MC_OFFSET_PHILOX + rng_offs_mc)
                lnS_1d      = current_lnS + drift_l2 + (vol_l2 * Z_1d)
                path_tile   = tl.exp2(lnS_1d)[:, None]
                current_lnS = lnS_1d
            else:
                ...
                lnS_2d      = current_lnS[:, None] + (drift_l2 * steps_2d) + (vol_l2 * Z_cumsum)
                path_tile   = tl.exp2(lnS_2d)
                last_mask   = (tl.arange(0, BLOCK_SIZE_DIM) == BLOCK_SIZE_DIM - 1)[None, :]
                current_lnS = tl.sum(lnS_2d * last_mask.to(tl.float32), axis=1)
        else:
        ...
        if BLOCK_SIZE_PARTICLES == 1 and BLOCK_SIZE_DIM == 1:
            ...
            discount    = tl.exp(-r * dt * (dim_offset + 1))
            ...
            discount_1d     = tl.exp(-r * dt * (dim_offset + first_ex_idx_1d + 1))
            ...
            discount_2d     = tl.exp(-r * dt * (dim_offset + first_ex_idx_2d + 1))
    ...
    terminal_discount = tl.exp(-r * dt * NUM_DIMENSIONS)
