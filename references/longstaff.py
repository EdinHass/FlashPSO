...
class LSMC_OpenCL(LongStaffBase):
    ...
    def __preCalc_gpu(self):     # St in shape of nPath by nPeriod    
        ...
        match self.inverseType:
            case "GJ": 
                prog = cl.Program(openCLEnv.context, open("./references/kernels/lsmc/knl_src_pre_calc_GaussJordan.c").read()%(self.mc.nPath, self.mc.nPeriod)).build()
                knl_preCalcAll = cl.Kernel(prog, 'preCalcAll_GaussJordan')
            case "CA":
                prog = cl.Program(openCLEnv.context, open('./references/kernels/lsmc/knl_src_pre_calc_ClassicAdjoint.c').read()%(self.mc.nPath, self.mc.nPeriod)).build()
    ...
    def __preCalc_gpu_optimized(self):
        ...
        kernel_src = open(f"./references/kernels/lsmc/knl_src_pre_calc_optimized.c").read()
